...
function EmployeeListTable({Employees,setEmployees,setshowUpdateForm,setemployeeData}) {  
  const deleteCustomer = async(customerId)=>{
    ...
    .then((response)=>{
        const new_userList = Employees.filter(employee => {
          return (employee._id != customerId)
        })
        console.log(new_userList)
        setEmployees(new_userList)
    })
    .catch((error)=>{
      console.log(error);
    })
